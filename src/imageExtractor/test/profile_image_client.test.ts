...
import type { ProfileImageRequest } from '../profile_image_client.js';
...
function requestReturning(
  ...
  overrides: Partial<ProfileImageRequest> = {},
): ProfileImageRequest {
  return {
...
test('returns the response text and mapped token usage', async () => {
  ...
  const result = await recognizeProfileImageWithModel(
    requestReturning({
      text: assessmentJson,
      usage: {
        promptTokens: 11,
...
test('reports an empty response', async () => {
  await assert.rejects(
    () =>
      recognizeProfileImageWithModel(requestReturning({ text: '' })),
...
test('passes the configured model, schema, timeout, and thinking', async () => {
  ...
  await recognizeProfileImageWithModel(
    requestReturning({ text: '{"ok":true}' }, {
...
test('does not require OPENROUTER_API_KEY when the model call is supplied', async () => {
...
test('reports token usage on a blocked response', async () => {
  await assert.rejects(
    ...
    (error: unknown) => {
      assert.ok(error instanceof ProfileImageModelError);
...
test('carries no usage when the model call itself failed', async () => {
  await assert.rejects(
    ...
    (error: unknown) => {
      ...
      assert.equal(error instanceof ProfileImageModelError, false);
      return true;
    },
  );
});
...
test('requires OPENROUTER_API_KEY when no model call is supplied', async () => {
  const previousKey = process.env['OPENROUTER_API_KEY'];
  delete process.env['OPENROUTER_API_KEY'];
